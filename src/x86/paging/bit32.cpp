#include "x86/cr.h"
#include "x86/cpuid.h"
#include "x86/paging/bit32.h"
#include "x86/paging/paging.h"

namespace x86::paging::bit32 {

bool pde_t::is_big() const {
    // assuming it's supported and enabled
    return big.ps == 1;
}

physical_address_t pde_t::address() const {
    if (is_big()) {
        return (static_cast<physical_address_t>(big.address2) << page_bits_4m) | pse36_address_bits();
    }

    return static_cast<physical_address_t>(small.address) << page_bits_4k;
}

void pde_t::address(const physical_address_t address) {
    if (is_big()) {
        big.address2 = (address >> page_bits_4m) & 0x3ff;
        pse36_address_bits(address);
    } else {
        small.address = static_cast<uint32_t>(address >> page_bits_4k);
    }
}

physical_address_t pde_t::pse36_address_bits() const {
    if (!is_pse36_supported()) {
        return 0;
    }

    const auto max_physical_address = max_physical_address_width();
    const physical_address_t mask = ((max_physical_address - 20) - 13 + 1) << 13;
    return (big.address2 & ~(1ull << mask)) << 32;
}

void pde_t::pse36_address_bits(const physical_address_t address) {
    if (!is_pse36_supported()) {
        return;
    }

    const auto max_physical_address = max_physical_address_width();
    const physical_address_t mask = ((max_physical_address - 20) - 13 + 1) << 13;
    big.address = (address >> 32) & ~(1ull << mask);
}

physical_address_t pte_t::address() const {
    return static_cast<physical_address_t>(bits.address) << page_bits_4k;
}

void pte_t::address(const physical_address_t address) {
    bits.address = static_cast<uint32_t>(address >> page_bits_4k);
}

bool are_4m_page_tables_supported() {
    // [SDM 3 4.1.4 P109]
    const auto cpuid = x86::cpuid<cpuid_eax01_t>();
    return cpuid.edx.bits.pse != 0;
}

bool are_4m_page_tables_enabled() {
    // [SDM 3 4.1.4 P109]
    if (!are_4m_page_tables_supported()) {
        return false;
    }

    const auto cr4 = read<cr4_t>();
    return cr4.bits.page_size_extensions != 0;
}

bool is_pse36_supported() {
    // [SDM 3 4.1.4 P109]
    const auto cpuid = x86::cpuid<cpuid_eax01_t>();
    return cpuid.edx.bits.pse36 != 0;
}

bool to_physical(const cr3_t& cr3, const linear_address_t address, physical_address_t& out) {
    const auto pde_address = (static_cast<physical_address_t>(cr3.bit32.address) << page_bits_4k)
            | (address.big.directory << 2);
    const auto pde = reinterpret_cast<const pde_t*>(pde_address);
    if (!pde->is_present()) {
        return false;
    }

    if (pde->is_big()) { // assuming that CR4.PSE is actually enabled
        out = pde->address() | (address.big.offset);
        return true;
    }

    const auto pte_address = pde->address() | (address.small.table << 2);
    const auto pte = reinterpret_cast<const pte_t*>(pte_address);
    if (!pte->is_present()) {
        return false;
    }

    out = pte->address() | (address.small.offset);
    return true;
}

}
